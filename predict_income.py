...
def training_data_preprocessing(dataset):
    ...
    dataset['Income in EUR'] = dataset['Income in EUR'].apply(np.log)
...
test_dataset = test_dataset.loc[:, ['Year of Record', 'Gender', 'Age', 'Country', 'Size of City', 'Profession', 'University Degree', 'Hair Color', 'Wears Glasses', 'Body Height [cm]', 'Income', ]]
...
test_dataset = test_dataset.rename(columns={"Income": "Income in EUR"})
...
training_dataset = training_dataset.drop("Income in EUR",1)
